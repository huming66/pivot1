(function() {
  ...
  callWithJQuery(function($) {
    ...
    aggregatorTemplates = {
      ...
      hm_count: function(formatter, vFlt) {
        if (formatter == null) {
          formatter = usFmt;
        }
        return function(arg) {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              push: function(record) {
                if (!isNaN(parseFloat(record[attr]))) {
                  return this.sum += vFlt(parseFloat(record[attr])) ? 1: 0;
                }
              },
              value: function() {
                return this.sum;
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              value: function() {
                ...
                if (q>=1) {  //mhu
                  var q1 = (spa.percentile.replaceAll(/[BbTt]/gi, '') ||95) / 100     
                  if (['B','b','T','t'].includes(spa.percentile[0])) q1 = 1-q1             
                  // i = (this.vals.length - 1) * +(spa?.percentile||0.95) 
                } else {q1 = q}
                if (['B','b'].includes(spa.percentile[0])) {
                  q1 = 1-q1
                } 
                i = (this.vals.length - 1) * q1
    ...
    aggregators = (function(tpl) {
      return {
        ...
        "count <0": tpl.hm_count(usFmtInt, (x) => {return x < 0}),  //mhu
        "count =0": tpl.hm_count(usFmtInt, (x) => {return x == 0}),  //mhu        
        "count >0": tpl.hm_count(usFmtInt, (x) => {return x > 0}),  //mhu        
        ...
        "percentile1": tpl.quantile(0.99,usFmt),
        "percentile2": tpl.quantile(0.98,usFmt),
        "percentile5": tpl.quantile(0.95,usFmt),
        "percentile10": tpl.quantile(0.90,usFmt),
        "percentile20": tpl.quantile(0.80,usFmt),
        "percentile_pct": tpl.quantile(1,usFmt),        
    ...
    PivotData = (function() {
      function PivotData(input, opts) {
        ...
        this.sorted = false;
        this.traceStyleData = opts.traceStyleData || {}           //mhu
        this.aggregators_ = opts.aggregators_ || {}           //mhu
        spa.dataPC = []                          //mhu
        PivotData.forEachRecord(this.input, this.derivedAttributes, (function(_this) {
          return function(record) {
            if (_this.filter(record)) {
              spa.dataPC.push(record)            //mhu
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      for (i in rowKeys) {
        ...
        for (j in colKeys) {
          ...
          if (spa.cfg?.valFormat?.includes('int')) td.textContent = td.textContent.replace(/\.[\d]+$/, ''); // td value formatting as full integer
    ...
    $.fn.pivot = function(input, inputOpts, locale) {
      ...
      if (inputOpts?.aggregators_) {
        Object.keys(inputOpts.aggregators_).forEach(k => {                        //mhu for percentile
          aggregators[k] = eval('aggregatorTemplates.' + inputOpts.aggregators_[k])  //mhu
        })
      }
      ...
      try {
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            subopts.traceStyleData = opts.traceStyleData  //mhu
            subopts.aggregators_ = opts.aggregators_  //mhu
